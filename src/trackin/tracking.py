    ...
    for t in range(t_max - 1):
        for i in range(len(points[t])):
            ...
            for j in range(len(points[t + 1])):
                ...
                x1 = (
                    G.nodes[f"D_{t}_{i}"]["displ_x"]
                    + G.nodes[f"D_{t}_{i}"]["x"]
                )
                y1 = (
                    G.nodes[f"D_{t}_{i}"]["displ_y"]
                    + G.nodes[f"D_{t}_{i}"]["y"]
                )
                ...
                if f"D_{t}_{i}" in G.nodes() and f"D_{t+1}_{j}" in G.nodes():
                    ...
                    if (
                        tracked
                        and G.nodes[f"D_{t}_{i}"]["track_no"]
                        == G.nodes[f"D_{t+1}_{j}"]["track_no"]
                        and G.nodes[f"D_{t}_{i}"]["track_no"] is not None
                        and G.nodes[f"D_{t+1}_{j}"]["track_no"] is not None
                    ):
                        score = score * coeff
                        ...
                        if score <= max_score:
                            G.add_edge(
                                f"D_{t}_{i}", f"D_{t+1}_{j}", weight=score
                            )

    G = add_stxy_nodes_and_edges(
        G,
        ...
        # else generate shortest_paths until the shortest path doesn't contain any D nodes
            ...
            while current_track <= max_num:
                ...
                graph.remove_nodes_from(
                    [n for n in path if graph.nodes[n]["type"] == "D"]
                )
            return tracks
...
def remove_all_successors(graph, node, exception_node):
    ...
    graph.remove_edges_from(
        ...
        if succ != exception_node and succ != "T"
    )
