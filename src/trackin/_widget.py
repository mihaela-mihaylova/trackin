...
remove_track_node_event = EventEmitter(
    source=None, type_name="remove_track_node"
)
...
delete_all_connections_event = EventEmitter(
    source=None, type_name="delete_all_connections"
)
...
def generate_positions_list(df, folder_to_save):
    ...
    for _, dft in df.groupby("tframe"):
        ...
        for _, row in dft.iterrows():
            if not shared_state.TRACKED:
                positions.append(
                    (row["y"], row["x"], row["displ_y"], row["displ_x"])
                )
            else:
                positions.append(
                    (
    ...
    with open(
        os.path.join(folder_to_save, f"{shared_state.SESSION_FILE}"), "w"
    ) as f:
        f.write("")
    shared_state.NUM_DET_PER_FRAME = [
        len(sublist) for sublist in shared_state.DATA
    ]
    ...
    for img in image_files:
        try:
            ...
            QMessageBox.critical(
                None, "Image Load Error", f"Could not open image {img}: {e}"
            )
...
def choose_folder():
    ...
    if folder_path:
        ...
        if images:
            viewer.layers.clear()
            viewer.add_image(
                images[shared_state.current_index],
                name=os.path.basename(image_files[shared_state.current_index]),
            )
...
def load_csv():
    ...
    if not images_loaded:
        QMessageBox.information(
            None,
            "Load Images First",
            "Please load the images before loading the CSV file.",
        )
        return
    ...
    if csv_path:
        ...
        shared_state.SESSION_FILE = (
            f"track_session_{csv_filename}_{timestamp}.csv"
        )
        ...
        shared_state.DATA = generate_positions_list(
            csv_data, shared_state.csv_folder_to_save
        )
        csv_loaded = True
...
def load_track_file():
    ...
    if not images_loaded or not csv_loaded:
        QMessageBox.information(
            None,
            "Load Images and Detections First.",
            "Please load the images and detections before loading the track file to add new tracks to.",
        )
        return
    ...
    if csv_path:
        ...
        shared_state.UPD_TRACK_FILE = (
            f"with_new_tracks_added_{csv_filename}_{timestamp}.csv"
        )
        ...
        try:
            ...
            if list(track_df.columns) != expected_columns:
                ...
                QMessageBox.critical(
                    None,
                    "CSV Format Error",
                    f"CSV file must have the following columns in order: {expected_columns}",
                )
                return

            # Check if the file is empty or only has a header
            if track_df.empty or (
                len(track_df) == 0 and not track_df.columns.empty
            ):
                QMessageBox.critical(
                    None,
                    "File Is Empty",
                    "The loaded file is empty or only contains a header.",
                )
                return
            ...
            complete_path_to_session_file = os.path.join(
                shared_state.csv_folder_to_save, shared_state.SESSION_FILE
            )
            check_session_file = (
                os.path.getsize(complete_path_to_session_file) == 0
            )
            print(
                f"path exists: {os.path.exists(os.path.join(shared_state.csv_folder_to_save, shared_state.SESSION_FILE))}"
            )
            print(
                f"size is 0: {os.path.getsize(os.path.join(shared_state.csv_folder_to_save, shared_state.SESSION_FILE)) == 0}"
            )
            ...
            if check_session_file:
                ...
                updated_track_df_path = os.path.join(
                    shared_state.csv_folder_to_save,
                    shared_state.UPD_TRACK_FILE,
                )
                ...
                updated_tracks_df = pd.concat(
                    [track_df, session_df], ignore_index=True
                )

                # Save the combined tracks to the new track file
                updated_tracks_df.to_csv(
                    os.path.join(
                        shared_state.csv_folder_to_save,
                        f"{shared_state.UPD_TRACK_FILE}",
                    ),
                    index=False,
                )
                show_info(
            ...
            QMessageBox.critical(
                None,
                "File Error",
                f"The selected file is not a valid CSV or is malformed: {e}",
            )
            return
        except Exception as e:
            QMessageBox.critical(
                None, "Error", f"An unexpected error occurred: {e}"
            )
...
def next_image(event=None):
    ...
    if images:
        shared_state.current_index = (shared_state.current_index + 1) % len(
            images
        )
        update_image()
        image_slider.image_index.value = (
            shared_state.current_index
        )  # Sync slider value
...
def add_white_border(image, border_size=1):
    ...
    if image.ndim == 2:  # Grayscale image
        new_image = (
            np.ones(new_image_shape[:2], dtype=image.dtype) * 255
        )  # White border
        new_image[border_size:-border_size, border_size:-border_size] = image
    else:  # RGB image
        new_image = (
            np.ones(new_image_shape, dtype=image.dtype) * 255
        )  # White border
        new_image[border_size:-border_size, border_size:-border_size, :] = (
            image
        )
...
def update_image():
    ...
    if images:
        ...
        bordered_image = add_white_border(
            images[shared_state.current_index], border_size=2
        )
    ...
    if csv_data is not None and shared_state.DATA is not None:
        ...
        frame_number = int(
            os.path.splitext(
                os.path.basename(image_files[shared_state.current_index])
            )[0]
        )
        ...
        if 0 <= frame_number < len(shared_state.DATA):
            ...
            if frame_data:  # Only overlay points if data is present
                columns = (
                    ["y", "x", "displ_y", "displ_x"]
                    if not shared_state.TRACKED
                    else ["y", "x", "displ_y", "displ_x", "track_no"]
                )
...
def overlay_points(frame_data):
    ...
    if track_index != -1:
        track_point = np.array(
            [points[track_index]]
        )  # Select only the current track point
    else:
        track_point = np.empty(
            (0, 2)
        )  # Empty array if no track point is present
    ...
    for t in range(len(shared_state.track)):
        ...
        if track_idx != -1:
            y, x = shared_state.DATA[t][track_idx][
                :2
            ]  # Get (y, x) coordinates
...
def delete_detection(layer, event=None, use_key=False):
    ...
    if not use_key:  # Called from mouse event
        ...
        if curr_track[shared_state.current_index] != -1:
            clicked_index = curr_track[
                shared_state.current_index
            ]  # Use the tracked index if available
        else:
...
def delete_det_by_key(layer):
    ...
    if shared_state.TRACKED:
        shared_state.DATA[shared_state.current_index][clicked_index] = [
            -1000000,
            -1000000,
            0,
            0,
...
def add_detection(layer, event):
    ...
    if event.button == 1 and "Shift" in event.modifiers:
        ...
        image_shape = images[
            shared_state.current_index
        ].shape  # This gets (height, width) for the image

        # Ensure the click is within the image bounds
        if (0 <= data_coords[0] < image_shape[0]) and (
            0 <= data_coords[1] < image_shape[1]
        ):
            ...
            curr_track = shared_state.track
            if curr_track[shared_state.current_index] != -1:
                track_index = curr_track[
                    shared_state.current_index
                ]  # Get the correct track index
            ...
            layer.border_width_is_relative = (
                False  # Ensure the border width is absolute
            )

            # Ensure the new point is added with the correct data in shared_state
            if shared_state.TRACKED:
                shared_state.DATA[shared_state.current_index].append(
                    [int(data_coords[0]), int(data_coords[1]), 0, 0, None]
                )
            else:
                shared_state.DATA[shared_state.current_index].append(
                    [int(data_coords[0]), int(data_coords[1]), 0, 0]
                )

            add_node_with_dummy_edges(
                node=f"D_{shared_state.current_index}_{shared_state.NUM_DET_PER_FRAME[shared_state.current_index]}",
            ...
            print(
                f"Added node D_{shared_state.current_index}_{shared_state.NUM_DET_PER_FRAME[shared_state.current_index]}."
            )
            ...
            print(
                f"Added new point at data coordinates: {new_point}, with size: {point_size}"
            )
...
def setup_keybindings():
    ...
    viewer.bind_key(
        "Right", handle_right
    )  # Right arrow key to move to the next image
    viewer.bind_key(
        "Left", handle_left
    )  # Left arrow key to move to the previous image
    viewer.bind_key(
        "D",
        lambda event: delete_detection(
            viewer.layers.selection.active, use_key=True
        ),
    )  # Trigger only for D key
    ...
    viewer.bind_key(
        "X", deleteAllConnections
    )  # delete all connections from a node, apart from the D-X edge
...
def trackin_main():
    ...
    layout.addWidget(
        choose_folder.native
    )  # Add the magicgui widget's native Qt widget
    ...
    for i, _ in enumerate(data):
        for j, _ in enumerate(data[i]):
            ...
            if data[i][j][0] != -1000000 and data[i][j][1] != -1000000:
                f.write(
                    f"{i},{data[i][j][0]},{data[i][j][1]},{data[i][j][2]},{data[i][j][3]}\n"
                )
    f.close()
...
def update_labels():
    ...
    if shared_state.G is not None:
        ...
        shared_state.NUM_DETS = (
            shared_state.G.number_of_nodes()
            - 2
            - 2 * (len(shared_state.DATA) - 1)
        )
        shared_state.NUM_CONN = shared_state.G.number_of_edges() - 2 * (
            len(shared_state.DATA) - 1
        )
        ...
        if dets_label is None:
            ...
            container.layout().addWidget(
                dets_label
            )  # Add the labels to the layout dynamically
